...
#[derive(Serialize)]
struct DependabotOrganizationSecretData {
...
struct DependabotOrganizationSecret_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<DependabotOrganizationSecretData>,
}

#[derive(Clone)]
pub struct DependabotOrganizationSecret(Rc<DependabotOrganizationSecret_>);

impl DependabotOrganizationSecret {
...
impl Referable for DependabotOrganizationSecret {
    fn extract_ref(&self) -> String {
        format!("{}.{}", self.0.extract_resource_type(), self.0.extract_tf_id())
    }
}

impl Resource for DependabotOrganizationSecret { }

impl ToListMappable for DependabotOrganizationSecret {
    type O = ListRef<DependabotOrganizationSecretRef>;
...
impl Resource_ for DependabotOrganizationSecret_ {
    fn extract_resource_type(&self) -> String {
        "github_dependabot_organization_secret".into()
    }
...
pub struct BuildDependabotOrganizationSecret {
    pub tf_id: String,
...
impl BuildDependabotOrganizationSecret {
    pub fn build(self, stack: &mut Stack) -> DependabotOrganizationSecret {
        let out = DependabotOrganizationSecret(Rc::new(DependabotOrganizationSecret_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(DependabotOrganizationSecretData {
...
pub struct DependabotOrganizationSecretRef {
    shared: StackShared,
    base: String,
}

impl Ref for DependabotOrganizationSecretRef {
...
impl DependabotOrganizationSecretRef {
    fn extract_ref(&self) -> String {
        self.base.clone()
    }
