...
struct CodespacesSecret_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<CodespacesSecretData>,
}

#[derive(Clone)]
pub struct CodespacesSecret(Rc<CodespacesSecret_>);
...
impl Referable for CodespacesSecret {
    fn extract_ref(&self) -> String {
        format!("{}.{}", self.0.extract_resource_type(), self.0.extract_tf_id())
    }
}

impl Resource for CodespacesSecret { }

impl ToListMappable for CodespacesSecret {
    type O = ListRef<CodespacesSecretRef>;
...
impl Resource_ for CodespacesSecret_ {
    fn extract_resource_type(&self) -> String {
        "github_codespaces_secret".into()
    }
...
pub struct BuildCodespacesSecret {
    pub tf_id: String,
...
impl BuildCodespacesSecret {
    pub fn build(self, stack: &mut Stack) -> CodespacesSecret {
        let out = CodespacesSecret(Rc::new(CodespacesSecret_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(CodespacesSecretData {
...
pub struct CodespacesSecretRef {
    shared: StackShared,
    base: String,
}

impl Ref for CodespacesSecretRef {
