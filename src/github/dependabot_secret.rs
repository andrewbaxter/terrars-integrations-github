...
struct DependabotSecret_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<DependabotSecretData>,
}

#[derive(Clone)]
pub struct DependabotSecret(Rc<DependabotSecret_>);
...
impl Referable for DependabotSecret {
    fn extract_ref(&self) -> String {
        format!("{}.{}", self.0.extract_resource_type(), self.0.extract_tf_id())
    }
}

impl Resource for DependabotSecret { }

impl ToListMappable for DependabotSecret {
    type O = ListRef<DependabotSecretRef>;
...
impl Resource_ for DependabotSecret_ {
    fn extract_resource_type(&self) -> String {
        "github_dependabot_secret".into()
    }
...
pub struct BuildDependabotSecret {
    pub tf_id: String,
...
impl BuildDependabotSecret {
    pub fn build(self, stack: &mut Stack) -> DependabotSecret {
        let out = DependabotSecret(Rc::new(DependabotSecret_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(DependabotSecretData {
...
pub struct DependabotSecretRef {
    shared: StackShared,
    base: String,
}

impl Ref for DependabotSecretRef {
